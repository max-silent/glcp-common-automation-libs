...
class AppProvision(UISession):
    ...
    def provision_app(self, app_id, region):
        ...
        """
        data = {"application_id": app_id, "region": region, "action": "PROVISION"}
        return self.session.post(self._get_path("provisions"), json=data)
    ...
    def unprovision_app(self, app_cust_id):
        ...
        if app_cust_id:
            return self.session.patch(
                self._get_path(f"provisions/{app_cust_id}"), json=data
            )
    ...
    def delete_provision(self, app_cust_id):
        ...
        :return:
        """
        return self.session.delete(self._get_path(f"provisions/{app_cust_id}"))
    ...
    def wait_for(self, cust_ids, status, iterations=20, delay=6):
        ...
        for iteration in range(1, iterations + 1):
            ...
            log.error(
                f"Few customers did not attain '{status}' status\n\n"
