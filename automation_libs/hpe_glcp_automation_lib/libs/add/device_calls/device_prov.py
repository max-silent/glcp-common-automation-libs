...
class NetworkStorageComputeDeviceProvisionHelper(object):
    ...
    def __init__(
        ...
        certs,
        hpe_device_url=None,
        aruba_device_url=None,
        activate_v2_device_url=None,
        devices_v2_url=None,
        mac_address=None,
    ):
        """
        ...
        self.devices_v2_url = devices_v2_url
    ...
    def make_device_provision_request(self):
        ...
        self.DEVICE_PROVISION_URL = "https://{}:{}".format(
            self.hpe_device_url, self.CLUSTER_PORT
        )
        if "STORAGE" in self.device_type:
            ...
            headers["X-ap-info"] = (
                self.serial_number + "," + "None" + "," + self.part_number
            )
        ...
        if "COMPUTE" in self.device_type:
            ...
            if self.serial_number == "noapinfo":
                headers["X-ap-info"] = None
            else:
                headers["X-ap-info"] = (
                    self.serial_number + "," + "None" + "," + self.part_number
                )
            ...
            headers["Content-Type"] = "application/json"
        if "SD_WAN_GW" in self.device_type:
            api_path = "provision"
            method = "post"
            headers["X-Type"] = "provision-update"
            headers["Content-Type"] = "application/json"
            headers["X-Mode"] = "SD_WAN_GW"
            if self.certs["type"] == "wrong_mode_no_x_ap_info":
                method = "get"
                api_path = "storage-provision"
                headers["X-Mode"] = "STORAGE"
            elif self.certs["type"] == "sdwan_device_wrong_mode_with_x_ap_info":
                self.make_entry_in_pods_hosts_file(
                    device_endpoint_ip=cluster_device_endpoint_ip,
                    device_endpoint_url=self.devices_v2_url,
                )
                self.DEVICE_PROVISION_URL = "https://{}:{}".format(
                    self.devices_v2_url, self.CLUSTER_PORT
                )
                api_path = "hpe-provision"
                headers["X-Mode"] = "SWITCH"
                headers["X-ap-info"] = (
                    self.serial_number + "," + self.mac_address + "," + self.part_number
                )
        test_params = {
            "path_params": [],
    ...
    def get(self, api_path, cert="", key="", ca_cert=False, headers={}):
        ...
        log.info(
            "\nProcessing GET on URL - {} with headers {} with cert: {}\n".format(
                path.join(self.DEVICE_PROVISION_URL, api_path), headers, cert
            )
        )
        response = requests.get(
            path.join(self.DEVICE_PROVISION_URL, api_path),
            cert=(cert, key),
            headers=headers,
            verify=device_ca_file,
        )
        return response
    ...
    def iap_add_call_fn(self):
        ...
        act_url = (
            "https://" + self.aruba_device_url + ":" + self.CLUSTER_PORT + "/provision"
        )
        ...
        log.info(
            "request from server for challenge {} {} {}".format(
                resp.status_code, resp.headers, session
            )
        )
        ...
        logging.info(
            "response from server for final provision {} {} {}".format(
                resp.headers, resp.status_code, session
            )
        )
        return resp
    ...
    def add_call_firmware(self, endpoint, x_type, fw_version):
        ...
        headers["Connection"] = "Keep-Alive"
        logging.info(
            "Processing POST on URL - {}, with headers - headers {}\n".format(
                act_url, headers
            )
        )
        resp = requests.post(act_url, verify=False, headers=headers)
        ...
        headers["Connection"] = "close"
        logging.info(
            "SENDING RESPONSE BACK TO SERVER FROM CLIENT WITH  headers {} and session {}".format(
                headers, session
            )
        )
        resp = requests.post(act_url, verify=False, headers=headers, data=data)
        logging.info(
            "\nresponse from server for final provision: {} \nheaders: {} \nstatus_code: {} session: {}".format(
                resp.text, resp.headers, resp.status_code, session
            )
        )
        return resp

    def make_switch_provision_request(self, api_path, x_type, additional_header={}):
        """
        Make device provision / firmware request for Switch device
        :param api_path: endpoint for switch device types
        :param x_type: provision-update for provision request, firmware-check for firmware request
        :param additional_header: pass additional headers for simulation or as per need [Optional]
        ...
        self.DEVICE_PROVISION_URL = "https://{}:{}".format(
            self.activate_v2_device_url, self.CLUSTER_PORT
        )
        log.info(
            "\nRESOLVED IP FOR DEVICE ENDPOINT AND MADE ENTRY IN ETC/HOSTS FILE FOR POD\n"
        )
    ...
    def est_provision_request(self):
        """
        To make /estprovision call for VGW and nontpm device
           :return: est provision response for VGW device.
        """
        cluster_device_endpoint_ip = self.resolve_device_hostname_to_IP()
        self.make_entry_in_pods_hosts_file(
            device_endpoint_ip=cluster_device_endpoint_ip,
        ...
        headers = {}
        act_url = (
            "https://" + self.aruba_device_url + ":" + self.CLUSTER_PORT + "/estprovision"
        )
        headers["X-Type"] = "provision-update"
        headers["Content-Type"] = "application/json"
        headers["X-Identity"] = self.serial_number
        headers[
            "X-Ap-Info"
        ] = f"{self.serial_number},{self.mac_address},{self.part_number}"
        headers["X-Salt"] = "9914e596c01a63d7"
        headers["X-Hello"] = "667ac610d6152f4c43e867b0d22741f045e716c7"
        log.info(
            "\nProcessing est provision call with : url: {} and headers: {}\n".format(
                act_url, headers
            )
        )
        resp = requests.post(act_url, verify=False, headers=headers)
        log.info(
            "\nAPI RESPONSE : status: {}, response_text: {}, response_headers: {}\n".format(
                resp.status_code, resp.text, resp.headers
            )
        )
        return resp

    def est_verify_challenge(self, username, payload_data):
        """
        To make /verifyestchallenge for VGW and nontpm device
           :param username: serial no, part no, mac and device model detail
           :param payload_data: payload data of device for request
        ...
        params = {"serial": self.serial_number}
        act_url = "https://" + self.aruba_device_url + "/api/ext/verifyestchallenge"
        verify_est_body = {
            "username": username,
            "realm": "estserver",
            "requri": "/.well-known/est/simpleenroll",
            "method": "POST",
            "qop": "auth",
        }
        verify_est_body.update(payload_data)
        log.info(
            "\nProcessing est verify challenge call with : url: {}, payload: {} and headers: {}\n".format(
                act_url, verify_est_body, headers
            )
        )
        resp = requests.post(
            act_url, data=verify_est_body, headers=headers, params=params, verify=False
        )
        logging.info(
            "response from server for est verify {} {} {}".format(
                resp.headers, resp.status_code, resp.text
            )
        )
        return resp
    ...
    def post(self, endpoint, cert="", key="", ca_cert=False, headers={}):
        ...
        log.info(
            "\nProcessing POST on URL - {}, with headers - {}, with cert: {}, with key: {}, with ca_cert: {}\n".format(
