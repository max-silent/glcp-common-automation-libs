...
class ReportCore(Session):
    ...
    def get_next_runtime(self, payload):
        ...
        return self.post(
            url=self._get_path("schedules/next-run-time"),
            json=payload,
            ignore_handle_response=True,
        )

    @_log_response
    def fetch_gatekeeper(self, payload):
        """
        Fetch Gatekeeper pass for report to be triggered
        :param payload: JSON data of the input paramaters
        :return: Signature of the gatepass
        """
        # http://report-core-svc.ccs-system.svc.cluster.local:80/report-core/internal/v1/gatekeeper/obtain-gatepass
        return self.post(
            url=self._get_path("gatekeeper/obtain-gatepass"),
            json=payload,
            ignore_handle_response=True,
        )
