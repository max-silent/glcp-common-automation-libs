...
class ActivateFolderDetails(HeaderedPage):
    ...
    def edit_folder(self, **kwargs):
        ...
        for key in kwargs.keys():
            if "name" in key:
                self.page.locator(
                    ActivateFolderDetailsSelectors.FOLDER_NAME_INPUT_FIELD
                ).fill(kwargs[key])
            elif "parent" in key:
                self.page.locator(
                    ActivateFolderDetailsSelectors.PARENT_FOLDER_MENU
                ).click()
                self._search_drop_down("searchbox", kwargs[key], "option")
            elif "description" in key:
                self.page.locator(
                    ActivateFolderDetailsSelectors.DESCRIPTION_INPUT_FIELD
                ).fill(kwargs[key])
            else:
                raise KeyError(
                    "Name of the new parameter does not fit the expected. Expected fields to change in a "
    ...
    def wait_for_loaded_table(self):
        ...
        self.page.wait_for_selector(
            ActivateFolderDetailsSelectors.TABLE_ROWS, state="visible", strict=False
        )
    ...
    def delete_folder(self):
        ...
        self.page.locator(ActivateFolderDetailsSelectors.OK_POPUP_BUTTON).wait_for(
            state="hidden"
        )
    ...
    def add_new_rule(self, rule_info: RuleData = RuleData()):
        ...
        self.page.locator(ActivateFolderDetailsSelectors.RULE_NAME_INPUT_FIELD).fill(
            rule_info.name
        )
        ...
        if rule_info.folder:
            self.page.locator(
                ActivateFolderDetailsSelectors.PARENT_FOLDER_DROP_DOWN
            ).click()
        ...
        if rule_info.email_to:
            self.page.locator(ActivateFolderDetailsSelectors.EMAIL_TO_INPUT_FIELD).fill(
                rule_info.email_to
            )
        if rule_info.prov_type:
            self.pw_utils.select_drop_down_element(
                ActivateFolderDetailsSelectors.PROVISIONING_TYPE_DROP_DOWN,
                rule_info.prov_type,
                "option",
            )
        if rule_info.amp_ip:
            self.page.locator(ActivateFolderDetailsSelectors.AMP_IP_INPUT_FIELD).fill(
                rule_info.amp_ip
            )
        if rule_info.shared_secret:
            self.page.locator(
                ActivateFolderDetailsSelectors.SHARED_SECRET_INPUT_FIELD
            ).fill(rule_info.shared_secret)
        if rule_info.organization:
            self.page.locator(
                ActivateFolderDetailsSelectors.ORGANIZATION_INPUT_FIELD
            ).fill(rule_info.organization)
        if rule_info.ap_group:
            self.page.locator(ActivateFolderDetailsSelectors.AP_GROUP_INPUT_FIELD).fill(
                rule_info.ap_group
            )
        if rule_info.controller:
            self.page.locator(ActivateFolderDetailsSelectors.CONTROLLER_INPUT_FIELD).fill(
                rule_info.controller
            )
        if rule_info.backup_contrlr_ip:
            self.page.locator(
                ActivateFolderDetailsSelectors.BACKUP_CONTROLLER_IP_INPUT_FIELD
            ).fill(rule_info.backup_contrlr_ip)
        if rule_info.conductor_mac:
            self.page.locator(
                ActivateFolderDetailsSelectors.CONDUCTOR_MAC_DROP_DOWN
            ).click()
            self._search_drop_down("searchbox", rule_info.conductor_mac, "menuitem")
        if rule_info.primary_conductor:
            self.page.locator(
                ActivateFolderDetailsSelectors.PRIMARY_CONTROLLER_DROP_DOWN
            ).click()
            self._search_drop_down("searchbox", rule_info.primary_conductor, "menuitem")
        if rule_info.backup_conductor:
            self.page.locator(
                ActivateFolderDetailsSelectors.BACKUP_CONDUCTOR_DROP_DOWN
            ).click()
            self._search_drop_down("searchbox", rule_info.backup_conductor, "menuitem")
        if rule_info.primary_ctrl_ip:
            self.page.locator(
                ActivateFolderDetailsSelectors.PRIMARY_CONTROLLER_IP_INPUT_FIELD
            ).fill(rule_info.primary_ctrl_ip)
        if rule_info.backup_ctrl_ip:
            self.page.locator(
                ActivateFolderDetailsSelectors.BACKUP_CTRL_IP_INPUT_FIELD
            ).fill(rule_info.backup_ctrl_ip)
        if rule_info.branch_ctrl_group:
            self.page.locator(
                ActivateFolderDetailsSelectors.BRANCH_CONFIG_GROUP_INPUT_FIELD
            ).fill(rule_info.branch_ctrl_group)
        if rule_info.redundancy_level:
            self.page.locator(
                ActivateFolderDetailsSelectors.REDUNDANCY_LEVEL_DROP_DOWN
            ).click()
            self.page.get_by_role("option", name=rule_info.redundancy_level)
        if rule_info.config_node_path:
            self.page.locator(
                ActivateFolderDetailsSelectors.CONFIG_NODE_PATH_INPUT_FIELD
            ).fill(rule_info.config_node_path)
        if rule_info.mobility_conductor:
            self.page.locator(
                ActivateFolderDetailsSelectors.MASTER_CONTROLLER_DROP_DOWN
            ).click()
            self._search_drop_down(
                "searchbox", rule_info.mobility_conductor, "menuitem", exact=True
            )
        if rule_info.mobility_conductor_ip:
            self.page.locator(
                ActivateFolderDetailsSelectors.MASTER_CONTROLLER_IP_INPUT_FIELD
            ).fill(rule_info.mobility_conductor_ip)
        if rule_info.backup_mobility_conductor:
            self.page.locator(
                ActivateFolderDetailsSelectors.BACKUP_CONDUCTOR_DROP_DOWN
            ).click()
            self._search_drop_down(
                "searchbox", rule_info.backup_mobility_conductor, "menuitem", exact=True
            )
        if rule_info.vpn_concentrator_mac:
            self.page.locator(
                ActivateFolderDetailsSelectors.VPN_CONCENTRATOR_MAC_DROP_DOWN
            ).click()
            self._search_drop_down(
                "searchbox", rule_info.vpn_concentrator_mac, "menuitem"
            )
        if rule_info.vpn_concentrator_ip:
            self.page.locator(
                ActivateFolderDetailsSelectors.VPN_CONCENTRATOR_IP_INPUT_FIELD
            ).fill(rule_info.vpn_concentrator_ip)
        if rule_info.backup_vpn_concentrator_mac:
            self.page.locator(
                ActivateFolderDetailsSelectors.BACKUP_VPN_CONDUCTOR_MAC_DROP_DOWN
            ).click()
            self._search_drop_down(
                "searchbox", rule_info.backup_vpn_concentrator_mac, "menuitem", exact=True
            )
        if rule_info.country:
            self.page.locator(
                ActivateFolderDetailsSelectors.COUNTRY_CODE_DROP_DOWN
            ).click()
    ...
    def delete_rule(self, rule_name):
        ...
        self.page.locator(
            ActivateFolderDetailsSelectors.RULE_LIST_ROW_TEMPLATE.format(rule_name)
    ...
    def search_for_text(self, search_text, ensure_not_empty=True):
        ...
        self.pw_utils.enter_text_into_element(
            ActivateFolderDetailsSelectors.SEARCH_DEVICE_INPUT_FIELD, search_text
        )
        if ensure_not_empty:
    ...
    def should_have_rule_in_the_list(self, rule_name):
        ...
        :return: current instance of Activate Folder Details page object.
        """
        rule_selector = ActivateFolderDetailsSelectors.RULE_LIST_ROW_TEMPLATE.format(
            rule_name
        )
