...
class ActivateDevices(HeaderedPage, SideMenuNavigablePage):
    ...
    def wait_for_loaded_table(self):
        ...
        self.page.wait_for_selector(
            ActivateDevicesSelectors.TABLE_ROWS, state="visible", strict=False
        )
        self.page.locator(ActivateDevicesSelectors.LOADER_SPINNER).wait_for(
            state="hidden"
        )
    ...
    def open_device_details(self, device_identifier, column_name="Serial Number"):
        ...
        self.wait_for_loaded_table()
        matching_row_index = self.table_utils.get_rows_indices_by_text_in_column(
    ...
    def export_allowed_list_cli(self, email):
        """
        Export Allowed List CLI for all workspace activate devices
        :param email: email to send CSV to.
        :return: current instance of Activate Devices page object.
        """
        name = "Allowed-list command lines"
        return self._export_csv_for_all_devices(email=email, item_name=name)

    def export_legacy_allowed_list_cli(self, email):
        """
        Export Legacy Allowed List CLI for all workspace activate devices
        :param email: email to send CSV to.
        :return: current instance of Activate Devices page object.
        """
        name = "Allowed-list legacy command lines"
        return self._export_csv_for_all_devices(email=email, item_name=name)

    def export_inventory_csv(self, email):
        """
        Export Inventory CSV for all workspace activate devices
        :param email: email to send CSV to.
        :return: current instance of Activate Devices page object.
        """
        name = "Inventory CSV"
        self._export_csv_for_all_devices(email=email, item_name=name)
        return self

    def export_allowed_list_csv(self, email):
        """
        Export Allowed List CSV for all workspace activate devices
        :param email: email to send CSV to.
        :return: current instance of Activate Devices page object.
        """
        name = "Allowed-list CSV"
    ...
    def get_devices_count(self):
        ...
        """
        return int(
            self.page.locator(ActivateDevicesSelectors.TABLE_SUMMARY_COUNT).text_content()
        )

    def close_success_notification(self):
        """
        Click at close button of 'notification-status-ok' popup notification.
        :return: current instance of Activate Devices page object.
        """
        log.info(
            "Playwright: click at 'notification-status-ok' close button at activate devices."
        )
        self.page.locator(ActivateDevicesSelectors.OK_NOTIF_CLOSE_BTN).click()
        return self
    ...
    def search_for_text(self, search_text):
        ...
        :return: current instance of Activate Devices page object.
        """
        log.info(f"Playwright: search for text: '{search_text}' at activate devices.")
        self.pw_utils.enter_text_into_element(
            ActivateDevicesSelectors.SEARCH_FIELD, search_text
        )
        self.wait_for_loaded_table()
        return self
    ...
    def move_device_to_another_folder(self, folder_name):
        ...
        self.pw_utils.enter_text_into_element(
            ActivateDevicesSelectors.SEARCH_TEXT_BOX, folder_name
        )
    ...
    def select_all_device_type_filter(self, device_type):
        ...
        self.pw_utils.enter_text_into_element(
            ActivateDevicesSelectors.SEARCH_TEXT_BOX, device_type
        )
        self.pw_utils.click_selector(ActivateDevicesSelectors.SELECT_OPTION)
        return self
    ...
    def select_all_model_type_filter(self, model_name):
        ...
        self.pw_utils.enter_text_into_element(
            ActivateDevicesSelectors.SEARCH_TEXT_BOX, model_name
        )
        self.pw_utils.click_selector(ActivateDevicesSelectors.SELECT_OPTION)
        return self
    ...
    def should_have_search_field(
        self, placeholder="Search by Serial, MAC, or Device Name"
    ):
        """
    ...
    def should_have_row_with_text_in_column(self, column_name, value):
        ...
        expect(
            self.page.locator(
                ActivateDevicesSelectors.TABLE_ROW_TEMPLATE.format(
    ...
    def should_have_rows_count(self, count):
        ...
        expect(self.page.locator(ActivateDevicesSelectors.TABLE_ROWS)).to_have_count(
            count
        )
        self.page.wait_for_load_state("domcontentloaded")
        return self
    ...
    def should_have_device_token_success_notification(self, text):
        ...
        expect(
            self.page.locator(
                ActivateDevicesSelectors.GENERATE_TOKEN_SUCCESS_NOTIFICATION
            )
        ).to_have_text(text)
        return self

    def should_have_export_success_notification(self):
        """
        Check that popup message about successful devices export is displayed.
        :return: current instance of Activate Devices page object.
        """
        log.info(
            "Playwright: check message that notification will be sent is displayed at activate devices."
        )
        expect(
            self.page.locator(ActivateDevicesSelectors.EXPORT_OK_NOTIFICATION)
        ).to_be_visible()
        return self

    # TODO: Remove this method and all methods where it's used when confirmed that downloading locally is not supported.
    def _download_csv_for_all_devices(self, item_name=""):
        ...
        log.info(
            f"Playwright: Download of the {item_name} takes {round(downloading_time, 4)} seconds"
        )
    ...
    def _export_csv_for_all_devices(self, email, item_name=""):
        """
        Send by email CSV with activated devices.
        :param email: email to send CSV to.
        :param item_name: Export list Item name.
        """
        log.info(
            f"Playwright: sending activated devices' '{item_name}' list to '{email}'."
        )
        self.page.locator(ActivateDevicesSelectors.BULK_ACTIONS_BTN).click()
        self.page.locator(ActivateDevicesSelectors.EXPORT_SELECTED_BTN).click()
        self.pw_utils.enter_text_into_element(
            ActivateDevicesSelectors.EMAIL_ADDR_FIELD, email
        )
        self.page.get_by_text(item_name).check()
        self.page.locator(ActivateDevicesSelectors.GENERATE_REPORT_BTN).click()
