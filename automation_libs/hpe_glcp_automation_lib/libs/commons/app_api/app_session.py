...
class AppSession(Session):
    ...
    def __reuse_session(self, client_details):
        ...
        log.info(
            f"App API session for {self.client_id} was accessed successfully for reusing."
        )
    ...
    def __purge_current_session(self):
        ...
        if session_purged:
            log.info(
                f"Stored App API session for {self.client_id} was purged successfully."
            )
        else:
            log.warning(
    ...
    def get_secondary_app_api_hostname(self):
        """Get secondary app api hostname for read-only DR cluster
        :return: app api hostname with -r

        """
        try:
            log.info("getting app api url")
            prefix_base_url = self.base_url.split(".")
            prefix_base_url[0] = self.base_url.split(".")[0] + "-r"
            return ".".join(prefix_base_url)
        except Exception:
            log.error("not able get app api url")
