...
from hpe_glcp_automation_lib.libs.authn.user_api.session.ui.login_factory import (
    CCSLoginFactory,
)
...
class UISession(Session):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        :param host: CCS UI Hostname with or without https:// as prefix
        ...
        self.host = self.ret_hostname(host)
        self.user = user
        ...
        self.redirect_uri = self.host + "/authentication/callback"
    ...
    def __store_current_session(self):
        self.stored_sessions[(self.host, self.user, self.password)] = {
            "token": None,
            "session": None,
        }
        self.stored_sessions[(self.host, self.user, self.password)][
    ...
    def __reuse_session(self, user_details):
        ...
        log.info(
            f"User API session for {self.user} was accessed successfully for reusing."
        )

    def __purge_current_session(self):
        session_purged = self.stored_sessions.pop(
            (self.host, self.user, self.password), False
        )
        if session_purged:
            log.info(f"Stored User API session for {self.user} was purged successfully.")
        else:
            log.warning(
                f"There is no stored User API session for {self.user}, but session purge was requested for it"
            )
    ...
    def __get_tokens(self, response):
        ...
        if "disconnected" in os.getenv("CURRENT_ENV", ""):
            self.token_json = self.post(
                self.sso_host + "/as/token.oauth2", data=data, verify=False
            )
        else:
    ...
    def __get_session(self):
        ...
        if "disconnected" in os.getenv("CURRENT_ENV", ""):
            return self.post("/authn/v1/session", json=data, verify=False)
        else:
    ...
    def _get_user_api_hostname(self):
        try:
            if os.getenv("POD_NAMESPACE") is None:
                log.info(
                    "running in local env, configmap is not avalaible, getting from settings.json"
                )
                if "disconnected" in os.getenv("CURRENT_ENV", ""):
                    cluster_config = self.get(self.host + "/settings.json", verify=False)
                else:
                    cluster_config = self.get(self.host + "/settings.json")
    ...
    def _get_secondary_user_api_hostname(self):
        """Get secondary user api hostname for read-only DR cluster
        :return: user api hostname with -r
        """
        try:
            log.info("getting read only url derived from settings.json")
            cluster_config = self.get(self.host + "/settings.json")
            prefix_base_url = cluster_config["baseUrl"].split(".")
            prefix_base_url[0] = prefix_base_url[0] + "-r"
            return ".".join(prefix_base_url)
        except Exception as e:
            log.error("not able to get read only url {}".format(e))
    ...
    def get_settings(self):
        if "disconnected" in os.getenv("CURRENT_ENV", ""):
            return self.get(self.host + "/settings.json", verify=False)
        else:
            return self.get(self.host + "/settings.json")
    ...
    def login(self):
        ...
        if user_details in self.stored_sessions:
            ...
            if "disconnected" in os.getenv("CURRENT_ENV", ""):
                response = self.get(
                    self.sso_host + "/as/authorization.oauth2",
                    params=query_params,
                    verify=False,
                )
            else:
                response = self.get(
                    self.sso_host + "/as/authorization.oauth2", params=query_params
                )
