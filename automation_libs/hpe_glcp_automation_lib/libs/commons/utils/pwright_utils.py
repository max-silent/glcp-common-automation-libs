...
class SaveVideo(object):
    def save_video(self, page):
        ...
        allure.attach.file(
            source=pic_location, name="video", attachment_type=AttachmentType.WEBM
        )
...
class SaveScreenshot(object):
    def save_screenshot(self, page, test_name):
        ...
        page.screenshot(
            path=pic_location + random_str + test_name + ".png", full_page=True
        )
        allure.attach.file(
            source=pic_location + random_str + test_name + ".png",
