...
class HumioClass:
    ...
    def __init__(self, **kwargs):
        ...
        self.client = HumioClient(
            base_url=self.base_url, repository=self.repository, user_token=self.user_token
        )
    ...
    def create_queryjob(
        ...
        **kwargs,
        ...
        """
        try:
            queryjob = self.client.create_queryjob(
                query_string,
                start,
                end,
                is_live,
...
class HumioHelper:
    @staticmethod
    def get_last_event_transaction_id(
        humio_session, search_query, search_duration_in_ms=3600000, timeout=10000
    ):
        """
        ...
        :return: transaction_id if found in log message or None otherwise
        """
        ...
        while current_time < timeout_time:
            ...
            if result:
                try:
                    ...
                    if "message" not in json_log.keys():
                        json_log = json.loads(json_log["log"])
                        if "message" not in json_log.keys():
                            log.info("unable to find message key")
                            continue
                except Exception:
                    log.info("Error : Json Decode error")
                    continue
                ...
                if transaction_id:
                    log.info(
                        f"Found logs in humio with transaction_id: '{transaction_id}'"
                    )
    ...
    @staticmethod
    def event_with_transaction_id_exists(
        humio_session,
        transaction_id,
        service_name,
        result_search_str,
        ...
        while current_time < timeout_time:
            ...
            for event in result:
                try:
                    ...
                    if "service_name" not in json_obj.keys():
                        json_obj = json.loads(json_obj["log"])
                        if "service_name" not in json_obj.keys():
                            log.info("unable to find service_name key")
                            continue
                except Exception:
                    log.info("Error : Json Decode error")
                    continue
