...
class Sel_Class(object):
    ...
    def restart_docker_sel_hub(self):
        ...
        os.system(
            "docker ps -a | grep selenium | awk '{print $1}' | xargs -L1 docker stop"
        )
    ...
    def get_browser_log(self, driver):
        ...
        self.process_browser_logs_for_network_events(browser_log)
        ...
        for entry in self.browser_logs:
            ...
            if (
                ...
                if browser_log["params"]["response"]["status"] != 200:
                    log.info(
                        "response status: %s"
                        % browser_log["params"]["response"]["status"]
                    )
                    if "timing" in browser_log["params"]["response"]:
                        try:
                            if (
                                browser_log["params"]["response"]["timing"]["sendEnd"]
                                and browser_log["params"]["response"]["timing"][
                                    "sendStart"
                                ]
                            ):
                                send_end = browser_log["params"]["response"]["timing"][
                                    "sendEnd"
                                ]
                                send_start = browser_log["params"]["response"]["timing"][
                                    "sendStart"
                                ]
                                log.info(
                            ...
                            if (
                                browser_log["params"]["response"]["timing"][
                                    "connectStart"
                                ]
                                and browser_log["params"]["response"]["timing"][
                                    "connectEnd"
                                ...
                                connect_start_time = browser_log["params"]["response"][
                                    "timing"
                                ]["connectStart"]
                                connect_end_time = browser_log["params"]["response"][
                                    "timing"
                                ]["connectEnd"]
                                # log.info("connect start time {}".format(connect_start_time))
                                # log.info("connect end time {}".format(connect_end_time))
                                log.info(
                                    "Diff of connect_end and connect_start {}".format(
