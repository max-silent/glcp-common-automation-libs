"""
CCS Unified API Session Library
"""
import json
import logging
import time
...
class UnifiedSession(Session):
    """
    UnifiedSession common API class
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.scope = scope
        self.base_url = f"https://{unified_api_host}"
    ...
    def __store_current_session(self):
        ...
        log.info(f"Unified API session for {self.client_id} was stored successfully.")
    ...
    def __reuse_session(self, client_details):
        ...
        log.info(
            f"Unified API session for {self.client_id} was accessed successfully for reusing."
        )
    ...
    def __purge_current_session(self):
        ...
        if session_purged:
            log.info(
                f"Stored Unified API session for {self.client_id} was purged successfully."
            )
        else:
            log.warning(
                f"There is no stored API session for {self.client_id}, but session purge was requested for it"
            )
