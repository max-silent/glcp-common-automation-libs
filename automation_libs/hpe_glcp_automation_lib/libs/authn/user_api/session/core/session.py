...
def exception_handler(func):
    ...
    def decorator(obj, *args, **kwargs):
        allowed_tries_count = obj.max_retries + 1
        for trying in range(allowed_tries_count):
            next_try_available = trying < obj.max_retries
            try:
                if obj.debug:
                    obj._log_request(func, *args, **kwargs)
                return func(obj, *args, **kwargs)
            except HTTPError as e:
                retriable_errors = getattr(
                    obj, "retriable_errors", DEFAULT_RETRIABLE_ERRORS
                )
                if e.response.status_code in retriable_errors:
                    ...
                    if hasattr(
                        obj, "validate_retriable_response"
                    ) and not obj.validate_retriable_response(e.response):
                        raise SessionException(
                            "Response validation failed for API "
                            f"'{e.response.request.method} "
                            f"url :'{e.response.request.url}' "
                            f"status code :'{e.response.status_code}' "
                            f"Resp :'{e.response.text}'",
                            e.response,
                        )
                    else:
                        if next_try_available:
                            LOG.error(
                                f"Waiting for {obj.retry_timeout} seconds before retrying..."
                            )
                    ...
                    if (
                        next_try_available
                        and hasattr(obj, "refresh_token")
                        and obj.refresh_token()
                    ):
                    ...
                    raise SessionException(
                        f"Non Retryable error: '{e.response.status_code}' for "
                        f"method: '{e.response.request.method} "
                        f"url:  '{e.response.request.url}' "
                        f"Resp: '{e.response.text}' ",
                        e.response,
                    )
...
class Session:
    ...
    @exception_handler
    def get_secondary(
        self, url, tuple_response=False, ignore_handle_response=False, **kwargs
    ):
        """
        HTTP GET method
        :param url:
