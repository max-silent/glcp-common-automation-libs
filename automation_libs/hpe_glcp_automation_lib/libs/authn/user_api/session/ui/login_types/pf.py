            ...
            if actionText:
                    ...
                    if "name" not in formInput.attrib.keys():
                        ...
                        if formInput.attrib["type"] == "text":
                            ...
                            if passwd_entered == False:
                                form.append((formInput.attrib["name"], self.password))
                                passwd_entered = True
                            else:
                            ...
                            if formInput.attrib.get("value") != None:
                                form.append(
                                    (formInput.attrib["name"], formInput.attrib["value"])
                                )
                            else:
                ...
                if action.lower() == "post":
                    if "disconnected" in os.getenv("CURRENT_ENV"):
                        LOG.debug(
                            "Action : %s, URL : %s, kwargs: verify=False" % (action, url)
                        )
                        response = self.session.post(
                            url, data=form, timeout=60, verify=False
                        )
                    else:
