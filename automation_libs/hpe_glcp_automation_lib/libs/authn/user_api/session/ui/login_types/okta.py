...
class Okta_SSO(Okta):
    ...
    def login(self, response):
        ...
        url = (
            "https://"
            + urlparse.urlparse(response.url).netloc
            + "/sso/idps/"
            + idp_id
        ...
        data_params_1 = {
            ...
            "RelayState": urlparse.unquote(input_list[1]["value"] + "%3Fokta_key%3D")
            + okta_key,
        }
...
class OktaMFA(Okta):
    ...
    def mfa_login(self, id):
        if self.device_secret_id is None:
            raise MFASecretKeyFailedException(
                "MFA Device Secret ID not given!!!, hence login failed"
            )
        otp = pt.TOTP(self.device_secret_id)
        payload = {"passCode": otp.now(), "stateToken": self.token}
