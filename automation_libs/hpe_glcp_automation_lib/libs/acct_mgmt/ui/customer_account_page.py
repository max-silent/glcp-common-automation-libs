...
class CustomerAccount(HeaderedPage):
    ...
    def add_customer(self, account_data: CreateUserData = CreateUserData):
        ...
        self.page.locator(AddCustomerSelectors.SEARCH_COUNTRY).type(
            account_data.country, delay=500
        )
        self.page.get_by_role(
            role="option", name=account_data.country, exact=True
        ).click()
        ...
        self.page.locator(AddCustomerSelectors.POSTAL_CODE).fill(account_data.postal_code)
    ...
    def export_customer_data(self, email):
        """
        Exports the customer account data to email

        :param email: email for exporting data
        :return: self
        """
        log.info(f"Export Customer Account Data to {email}.")
        self.page.locator(CustomerAccountSelectors.ACTIONS_BTN).click()
        self.page.locator(CustomerAccountSelectors.EXPORT_BTN).click()
        self.page.fill(CustomerAccountSelectors.EXPORT_EMAIL_INPUT, email)
