...
class TacDeviceDetailsPage(HeaderedPage):
    ...
    def wait_for_loaded_details(self):
        """Wait for device details info is loaded.

        :return: current instance of TAC Device Details page object.
        """
        log.info("Playwright: wait for details are loaded at device details page.")
        self.pw_utils.wait_for_selector(
            TacDeviceDetailsSelectors.NO_DATA_SUBTITLE,
            state="visible",
            timeout_ignore=True,
            timeout=5000,
            strict=False,
        ...
        self.pw_utils.wait_for_selector(
            TacDeviceDetailsSelectors.NO_DATA_SUBTITLE, state="hidden", strict=False
        )
        return self
    ...
    def should_have_mac_address(self, mac_addr):
        ...
        expect(
            self.page.locator(TacDeviceDetailsSelectors.MAC_ADDRESS_VALUE)
        ).to_have_text(mac_addr)
        return self
    ...
    def should_have_serial(self, serial):
        ...
        expect(
            self.page.locator(TacDeviceDetailsSelectors.SERIAL_NUMBER_VALUE)
        ).to_have_text(serial)
        return self
    ...
    def should_have_part_no(self, part_no):
        ...
        expect(
            self.page.locator(TacDeviceDetailsSelectors.PART_NUMBER_VALUE)
        ).to_have_text(part_no)
        return self
    ...
    def should_have_assigned_folder(self, folder_name):
        ...
        """
        log.info(
            f"Playwright: check that device has correct folder name '{folder_name}' at device details page."
        )
        expect(
            self.page.locator(TacDeviceDetailsSelectors.FOLDER_NAME_VALUE)
        ).to_have_text(folder_name)
        return self
    ...
    def should_have_assigned_pcid(self, pcid):
        ...
        """
        log.info(
            f"Playwright: check that device has correct customer ID '{pcid}' at device details page."
        )
