...
class TacDevicesPage(HeaderedPage, TacMenuNavigablePage):
    ...
    def wait_for_loaded_table(self):
        ...
        self.page.wait_for_selector(
            TacDevicesSelectors.TABLE_ROWS, state="visible", strict=False
        )
    ...
    def search_for_text(self, search_text):
        ...
        self.pw_utils.enter_text_into_element(
            TacDevicesSelectors.SEARCH_FIELD, search_text
        )
        self.wait_for_loaded_table()
        return self

    def move_devices_to_customer_folder(
        self, devices_ids_list: List, pcid, folder_name, expect_failure=False
    ):
        ...
        self.page.locator(TacDevicesSelectors.SERIALS_OR_MACS_FIELD).fill(
            devices_ids_text
        )
        ...
        self.pw_utils.select_drop_down_element(
            drop_menu_selector=TacDevicesSelectors.FOLDER_SELECT_DROPDOWN,
            element=folder_name,
            element_role="option",
            exact_match=True,
        )
    ...
    def open_device_details_page(self, column_name, value):
        ...
        if column_name == "Serial Number":
            ...
            sn_column_selector = TacDevicesSelectors.TABLE_ROW_COLUMN_TEMPLATE.format(
                matching_rows_indices[0], sn_column_index
            )
            serial_number = self.page.locator(sn_column_selector).text_content()
        self.page.locator(
            TacDevicesSelectors.TABLE_ROW_TEMPLATE.format(matching_rows_indices[0])
        ).click()
    ...
    def should_have_error_containing_text(self, error_text):
        ...
        expect(self.page.locator(TacDevicesSelectors.ERROR_MESSAGE_AREA)).to_contain_text(
            error_text
        )
        return self
