...
class TacMenuNavigablePage(BasePage):
    ...
    def click_menu_link(self, menu_item_text):
        ...
        """
        log.warning(
            f"THIS METHOD IS DEPRECATED AND GOING TO BE REMOVED SOON."
            f"CONSIDER USING OF METHODS UNDER 'TacMenuNavigablePage().side_menu' ATTRIBUTE INSTEAD."
        )
        log.info(
            f"Playwright: navigate to page by menu link with text '{menu_item_text}'."
        )
        self.page.locator(
            TacSideMenuSelectors.MENU_LINK_TEMPLATE.format(menu_item_text)
        ).click()
